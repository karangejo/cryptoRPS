...
class CheckAndPay extends Component {
  state = {
    errorMessage: '',
    loading: false,
    message: '',
  ...
  onSubmit = async (event) => {
    ...
    try{
      ...
      const status = 'Succesfull Payout!'
  ...
  render(){
    return(
     ...
     <h3>Check Who Won and Payout<br/><small>(can only be called after both Players Have Finalized)</small></h3>
     <Form onSubmit={this.onSubmit} error={!!this.state.errorMessage}>
      {this.state.showSuccess && this.renderMessage()}
     <Message error header="Oops!" content={this.state.errorMessage}/>
     <Button loading={this.state.loading} primary>Check Winner</Button>
     </Form>
     </Segment>
     </div>
    );
